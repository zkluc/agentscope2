# -*- coding: utf-8 -*-
"""Examples of Agnes model calls via OpenAI-compatible API."""
import asyncio
import json
import os
...
async def example_simple_call() -> None:
    """Call the Agnes model with a simple text message."""
...
async def example_tool_call() -> None:
    """Call the Agnes model with tool calling enabled."""
...
async def example_structured_output() -> None:
    """Call the Agnes model and force a structured (JSON) output."""
    model = OpenAIChatModel(
        credential=AgnesCredential(
            api_key=os.environ["AGNES_API_KEY"],
            base_url="https://apihub.agnes-ai.com/v1",
